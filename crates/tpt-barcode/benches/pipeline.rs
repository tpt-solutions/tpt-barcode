...
fn bench_encode(c: &mut Criterion) {
    c.bench_function("encode_qr_v5m_100b", |b| {
        ...
        b.iter(|| {
            tpt_barcode::qr::encode(
                std::hint::black_box(&payload),
                tpt_barcode::core::EcLevel::M,
            )
        })
    });
    c.bench_function("encode_datamatrix_50b", |b| {
        let payload: Vec<u8> = (0..50u32).map(|i| b'a' + (i % 26) as u8).collect();
        b.iter(|| tpt_barcode::two_d::datamatrix::encode(std::hint::black_box(&payload)))
    });
...
fn bench_decode(c: &mut Criterion) {
    ...
    c.bench_function("decode_qr_grid_v1m", |b| {
        b.iter(|| tpt_barcode::two_d::qr::decode_grid(std::hint::black_box(&qr.matrix), qr.size))
    });
}
