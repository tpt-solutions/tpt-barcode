//! PDF417 codeword pattern tables and start/stop patterns.
//!
//! `CODEWORD_TABLE` maps `(cluster, codeword)` to a 17-module pattern stored
//! as an MSB-first bitmask (bit 16 = first module, 1 = bar). Transcribed from
//! the normative codeword table of ISO/IEC 15438:2001(E) as published in the
//! Apache-2.0-licensed ZXing reference implementation (`PDF417.java`).

/// Start pattern (17 modules, MSB first).
pub const START_PATTERN: u32 = 0x1fea8;
/// Stop pattern (18 modules incl. the 1-module termination bar, MSB first).
pub const STOP_PATTERN: u32 = 0x3fa29;

/// `CODEWORD_TABLE[cluster][codeword]` → 17-module pattern bitmask.
///
/// `cluster` is the row index mod 3 (nominal cluster values 0, 3, 6).
pub const CODEWORD_TABLE: [[u32; 929]; 3] = [
    [
...
mod tests {
    ...
    #[test]
    fn patterns_are_distinct_per_cluster() {
        for cluster in CODEWORD_TABLE.iter() {
            let mut sorted = cluster.to_vec();
            sorted.sort();
            sorted.dedup();
            assert_eq!(sorted.len(), 929);
        }
    }
}
